...
class Truncation(Enum):
  UNSET = 255  # -1 as uint8, but in xml files the value '99' is used!
  IN_IMAGE = 0
  TRUNCATED = 1
  OUT_IMAGE = 2
  BEHIND_IMAGE = 3

truncFromText = {'99':Truncation.UNSET, '0':Truncation.IN_IMAGE, '1':Truncation.TRUNCATED, '2':Truncation.OUT_IMAGE, '3': Truncation.BEHIND_IMAGE}
...
def parseXML(base_path, date, drive):
  ...
  _drive = date + '_drive_' + drive + '_sync'
  #_calib_path = os.path.join(base_path, date)
  trackletFile = os.path.join(base_path, date, _drive, 'tracklet_labels.xml')
        ...
        if info.tag == 'objectType':
          ...
          for pose in info:
            ...
            if pose.tag == 'count':   # this should come before the others
              ...
              newTrack.truncs = [Truncation.UNSET] * newTrack.nFrames #np.nan * np.ones((newTrack.nFrames), dtype='uint8')
              ...
              for poseInfo in pose:
                ...
                if poseInfo.tag == 'tx':
                  ...
                  newTrack.truncs[frameIdx] = truncFromText[poseInfo.text] # np.uint8(truncFromText[poseInfo.text])
