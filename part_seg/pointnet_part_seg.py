...
def get_loss(l_pred, seg_pred, label, seg, weight, end_points, part_num):
    ...
    one_hot_seg = tf.one_hot(seg, depth=part_num, axis=-1)
    #print(one_hot_seg.get_shape())
    if part_num == 2:
        ratio = 393162. / (393162. + 10223670.)
        class_weight = tf.constant([ ratio, 1. - ratio])
        print("Class weights:", class_weight)
    else:
        class_weight = (1. / part_num ) * tf.ones((part_num), dtype=tf.float32, name=None)
    
    #print(class_weight.get_shape())
    seg_pred_weighted = tf.multiply(class_weight, seg_pred) # shape [batch_size, 2]
    seg_pred = seg_pred_weighted
    #print(seg_pred.get_shape())
    #print(seg_pred_weighted.get_shape())
    #seg_pred_weighted = seg_pred

    per_instance_seg_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=seg_pred_weighted, labels=one_hot_seg), axis=1)
    #per_instance_seg_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=seg_pred, labels=seg), axis=1)
    #6438833 / 180303 
    #ratio = 0.5 # 180303 / (6438833 + 180303)
    #class_weight = tf.constant([ 1.0 - ratio, ratio])
    #class_weight = (1 / part_num ) * tf.ones((1,part_num), dtype=tf.float32, name=None)


    #weighted_logits = tf.multiply(seg_pred, class_weight) # shape [batch_size, 2]
    #per_instance_seg_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=seg_pred, labels=seg), axis=1)
    #per_instance_seg_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=weighted_logits, labels=one_hot_seg_pred), axis=1)
