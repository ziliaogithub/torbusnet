...
if split == 0: TRAIN_FILES = TEST_FILES
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss = MODEL.get_loss_didi_regression(pred, labels_pl)
        ...
        #sess = tf.Session(target="grpc://192.168.0.12:41153", config=config)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, current_label = provider_didi.loadDataFile(TEST_FILES[fn],NUM_POINT)
    ...
    log_string('eval mean loss: %f' % (loss_sum / float(len(TEST_FILES))))
